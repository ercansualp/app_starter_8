<template>
  <aside class="w-64 bg-sidebar text-white flex flex-col p-4 shadow-lg">
    ...
    <div class="mb-8 mt-2">
      <div class="relative">
        <button 
          ...
          class="w-full flex items-center justify-between p-3 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg bg-primary/10 hover:bg-primary/20 transition-colors duration-200 border-border-color dark:border-primary/20"
        >
          <div class="flex items-center min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1 text-left">
              <p class="text-text-primary dark:text-white font-semibold text-sm truncate">{{ currentProject?.name || 'Proje Seçin' }}</p>
              <p class="text-text-secondary dark:text-primary/70 text-xs">{{ currentProject ? 'Aktif Proje' : 'Proje yok' }}</p>
            </div>
          </div>
          <svg 
            ...
            class="text-text-secondary dark:text-primary/70 transition-transform duration-200 flex-shrink-0"
          ...
          >
            ...
            <div class="p-2">
              <div 
                ...
                :class="{
                  'bg-primary/10 text-primary dark:bg-primary dark:text-white': currentProject?.id === project.id,
                  'hover:bg-primary/30 dark:hover:bg-primary/80': currentProject?.id === project.id, /* Daha belirgin hover efekti */
                  'hover:bg-gray-100 dark:hover:bg-gray-700': currentProject?.id !== project.id /* Aktif olmayan projeler için hover */
                }"
              >
                <img 
                  v-if="project.logo" 
            ...
            <div class="p-2">
              <NuxtLink 
                ...
                class="flex items-center p-3 rounded-lg cursor-pointer transition-colors duration-200 hover:bg-background text-text-primary"
              >
    ...
    <nav class="flex-1">
      <ul>
        <li class="mb-2">
          <NuxtLink to="/" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
        ...
        <li class="mb-2">
          <NuxtLink to="/scheduler" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
        ...
        <li class="mb-2">
          <NuxtLink to="/message-templates" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
        ...
        <li class="mb-2">
          <NuxtLink to="/projects" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-3"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>
            Proje Yönetimi
          </NuxtLink>
        </li>
        <li class="mb-2">
          <NuxtLink to="/payment-methods" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
        ...
        <li class="mb-2">
          <NuxtLink to="/roles-permissions" class="flex items-center p-3 rounded-lg text-text-secondary hover:bg-gray-100 hover:text-text-primary dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200" active-class="bg-primary/10 text-primary dark:bg-primary dark:text-white">
...
const availableProjects = computed(() => {
  // Kullanıcının sahip olduğu veya erişimi olan projeler
...
const selectProject = (project) => {
  ...
  // Sayfa yenilenmesi gerekiyorsa (proje değişikliği diğer bileşenleri etkiliyorsa)
  // window.location.reload() veya router.go(0) kullanılabilir
}
  ...
  if (process.client) {
    ...
    if (savedProjectId) {
      ...
      if (project) {
        ...
        // Kaydedilen proje bulunamazsa varsayılan projeyi seç
      ...
      // Hiç proje seçilmemişse varsayılan projeyi seç
      const defaultProject = projects.value.find(p => p.isDefaultProject)
      if (defaultProject) {
        currentProject.value = defaultProject
        localStorage.setItem('selectedProjectId', defaultProject.id)
